...
export function parseXsd(xsdContent: string): XsdSchema {
  ...
  if (!xsdPrefix) {
    console.warn(
      "Warning: Could not detect XSD namespace prefix. Schema parsing may fail.",
    );
  }

  const elements = parseElements(xsd, xsdPrefix);
  const complexTypes = parseComplexTypes(xsd, xsdPrefix);
  const simpleTypes = parseSimpleTypes(xsd, xsdPrefix);

  const annotation = getTypeValue(xsd, "annotation", xsdPrefix);
  const annotationSimpleTypes = annotation
    ? parseSimpleTypesFromAnnotation(annotation, xsdPrefix)
    : [];
...
function findSchemaElement(parsed: any): any {
  const schemaKeys = Object.keys(parsed).filter(
  ...
  if (schemaKeys.length > 1) {
    console.warn(
      `Multiple schema elements found: ${schemaKeys.join(", ")}. Using first.`,
    );
    return parsed[schemaKeys[0]];
  }
  return parsed;
...
function parseSimpleTypesFromAnnotation(
  annotation: any,
...
function parseElement(
  ...
  const element: XsdElement = {
    ...
    maxOccurs:
      el.maxOccurs === "unbounded"
        ? "unbounded"
        : el.maxOccurs !== undefined
          ? parseInt(el.maxOccurs)
          : undefined,
    isRef: !!el.ref,
    ref: el.ref,
  ...
  if (complexType) {
    element.complexType =
      parseComplexType(complexType, xsdPrefix, el.name) ?? undefined;
  }

  const simpleType = getTypeValue(el, "simpleType", xsdPrefix);
  if (simpleType) {
    element.simpleType =
      parseSimpleType(simpleType, xsdPrefix, el.name) ?? undefined;
  }
  ...
  if (complexContent || simpleContent) {
    ...
    if (ext) {
      ...
      complexType.extension = {
        ...
        sequence: extSequence
          ? parseNestedElements(extSequence, complexType, xsdPrefix)
          : undefined,
...
function parseNestedElements(
  container: any,
  parentComplexType?: XsdComplexType,
  xsdPrefix?: string,
): XsdElement[] {
  const elements: XsdElement[] = [];
  ...
  for (const ch of choiceArray) {
    ...
    const choiceElements = parseNestedElements(
      ch,
      parentComplexType,
      xsdPrefix,
    );
    for (const choiceElement of choiceElements) {
      if (
        choiceElement.complexType &&
        Object.keys(choiceElement.complexType).length > 1
      ) {
...
function parseSimpleType(
  ...
  return {
    ...
    restriction: {
      ...
      minInclusive: parseFacetValue(
        restriction,
        "minInclusive",
        parseFloat,
        xsdPrefix,
      ),
      maxInclusive: parseFacetValue(
        restriction,
        "maxInclusive",
        parseFloat,
        xsdPrefix,
      ),
      minExclusive: parseFacetValue(
        restriction,
        "minExclusive",
        parseFloat,
        xsdPrefix,
      ),
      maxExclusive: parseFacetValue(
        restriction,
        "maxExclusive",
        parseFloat,
        xsdPrefix,
      ),
      totalDigits: parseFacetValue(
        restriction,
        "totalDigits",
        parseInt,
        xsdPrefix,
      ),
      fractionDigits: parseFacetValue(
        restriction,
        "fractionDigits",
        parseInt,
        xsdPrefix,
...
function parseAttributes(
  ...
  for (const attr of attrArray) {
    ...
    attributes.push({
      ...
      fixed: attr.fixed,
      simpleType: simpleTypeValue
        ? (parseSimpleType(simpleTypeValue, xsdPrefix, attr.name) ?? undefined)
        : undefined,
    });
  }
