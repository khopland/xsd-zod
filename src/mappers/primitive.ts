...
const PRIMITIVE_TYPE_MAP: Record<string, TypeMapping> = {
  ...
  positiveInteger: {
    tsType: "number",
    zodValidator: "z.number().int().positive()",
  },
  nonNegativeInteger: {
    tsType: "number",
    zodValidator: "z.number().int().nonnegative()",
  },
  negativeInteger: {
    tsType: "number",
    zodValidator: "z.number().int().negative()",
  },
  nonPositiveInteger: {
    tsType: "number",
    zodValidator: "z.number().int().nonpositive()",
  },
...
export function mapPrimitiveType(xsdType: string): TypeMapping {
  ...
  const mapping = PRIMITIVE_TYPE_MAP[normalizedType] || {
    tsType: "any",
    zodValidator: "z.any()",
  };
  return { ...mapping };
...
export function mapEnumeration(
