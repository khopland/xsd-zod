...
import { mapPrimitiveType, applyFacets, mapEnumeration, type TypeMapping } from './primitive';
import { applyNaming, getTypeName, getSchemaName, type NamingConvention } from '../generators/naming';
...
export function mapComplexType(
  ...
  schema: any
): { typeName: string; schemaName: string; tsInterface: string; zodObject: string } {
  ...
  const typeName = complexType.name ? getTypeName(complexType.name, naming) : 'AnonymousType';
  const schemaName = complexType.name ? getSchemaName(complexType.name, naming) : 'anonymousSchema';

  const allFields = [...elements, ...attributes];

  const tsInterface = allFields.length > 0
    ? `{\n${allFields.map(f => `  ${f.name}${f.optional ? '?' : ''}: ${f.tsType}`).join(',\n')}\n}`
    : '{}';

  const zodObject = allFields.length > 0
...
export function mapElement(
  ...
  if (element.complexType) {
    ...
    typeMapping = { tsType: complex.typeName, zodValidator: complex.schemaName };
    ...
    if (customType) {
      typeMapping = { tsType: getTypeName(customType.name, naming), zodValidator: getSchemaName(customType.name, naming) };
    } else {
...
function findCustomType(typeName: string, schema: any): any {
  const complexType = schema.complexTypes.find((ct: any) => ct.name === typeName);
