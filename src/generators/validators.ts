...
export function generateValidators(schema: XsdSchema, naming: NamingConvention): string {
  ...
  const simpleTypeValidators = generateSimpleTypeValidators(schema, naming, sortedSimpleTypes);
  const complexTypeValidators = generateComplexTypeValidators(schema, naming, sortedComplexTypes);
...
function topologicalSortComplexTypes(schema: XsdSchema, naming: NamingConvention): typeof schema.complexTypes {
  const visited = new Set<string>();
  const visiting = new Set<string>();
  const result: any[] = [];
  const nameToIndex = new Map<string, number>();
  ...
  function visit(complexType: any) {
    ...
    if (visiting.has(complexType.name)) {
      console.warn(`Circular dependency detected involving ${complexType.name}`);
      return;
    }
...
function generateSimpleTypeValidators(schema: XsdSchema, naming: NamingConvention, sortedSimpleTypes?: typeof schema.simpleTypes): string {
...
function generateComplexTypeValidators(schema: XsdSchema, naming: NamingConvention, sortedComplexTypes?: typeof schema.complexTypes): string {
  const validators: string[] = [];
