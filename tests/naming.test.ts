...
import { applyNaming, getTypeName, getSchemaName } from '../src/generators/naming';
...
describe('Naming Convention Utilities', () => {
  ...
  describe('getSchemaName', () => {
    ...
    it('should keep original + Schema for original naming', () => {
      expect(getSchemaName('user_profile', 'original')).toBe('user_profileSchema');
      expect(getSchemaName('UserProfile', 'original')).toBe('UserProfileSchema');
      expect(getSchemaName('user-profile', 'original')).toBe('user-profileSchema');
    });
